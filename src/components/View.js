...
const View = (props) =>
	<div className='container d-flex flex-column justify-content-center'>
		...
		<table className='m-3'><tbody>
			...
			<tr><td>Mme / M.        </td><td>{ props.name    }</td></tr>
			<tr><td>Né(e) le&nbsp;: </td><td>{ props.birth   }</td></tr>
			<tr><td>Demeurant&nbsp;:</td><td>{ props.address }</td></tr>
		</tbody></table>
		...
		<div className='m-3'>
			{
				SITUATIONS.map((descr, i) =>
					<div className='form-check' key={ i }>
						<input className='form-check-input' type='checkbox' readOnly checked={ Number(props.situation) === i } />
		...
		<p className='text-right'>Fait à { props.city }, le { today() }</p>
		<div className='text-right'><img height='80vh' src={ props.sign } alt='signature'/></div>
	</div>;
